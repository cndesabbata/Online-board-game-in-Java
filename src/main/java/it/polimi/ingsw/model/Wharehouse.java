...
public class Wharehouse {
    ...
    public void addResource(Resource resource, NumOfShelf numOfShelf) {
        ResourceQuantity shelf = wharehouse.get(numOfShelf.ordinal());
        if (shelf.getResource() != Resource.EMPTY && shelf.getResource() != resource) {/*gestisci caso risorse diverse nello stesso scaffale*/} else if (shelf.getQuantity() == 1 + numOfShelf.ordinal()) { /*gestisci caso scaffale pieno*/ } else if (!this.checkOtherShelves(resource, numOfShelf)) {/*gestisci caso altri scaffali hanno stessa risorsa*/} else
            shelf.setQuantity(shelf.getQuantity() + 1);
    }

    public void removeResource(Resource resource) {
        NumOfShelf numOfShelf = this.findShelf(resource);
        if (numOfShelf == NumOfShelf.NOT_FOUND) { /*gestisci caso scaffale non trovato*/} else {
            ResourceQuantity shelf = wharehouse.get(numOfShelf.ordinal());
